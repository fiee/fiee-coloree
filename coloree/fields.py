...
class HtmlColorCodeField(models.CharField):
    """
    A CharField that checks that the value
    is a valid HTML color code (Hex triplet).
    Has no required argument.
    Uses `validators.is_html_color_code` for validation.
...
try:
    ...
    add_introspection_rules([], ["^coloree\.fields\.HtmlColorCodeField"])
except ImportError:
    pass
